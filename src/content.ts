...
function openSearchPanel(
  ...
  document.addEventListener('tidl-playback-state', ((e: CustomEvent<{ trackId: string | null; state: string }>) => {
    ...
    const overlays = tidalPanel.querySelectorAll<HTMLDivElement>('.tidp-art-overlay');
...
function markFavoritedButtons(listEl: HTMLUListElement, favIds: Set<string>): void {
  for (const btn of Array.from(listEl.querySelectorAll<HTMLButtonElement>('.tidp-btn-fav[data-track-id]'))) {
    ...
    if (favIds.has(trackId) && !btn.classList.contains('favorited')) {
      setFavoriteButtonStateInline(btn, true, { animate: false });
    }
  }
}
...
function setFavoriteButtonStateInline(
